...
Each registered function gets one file at `docs/leaves/<leaf>.md`. The leaf name is the function id's suffix after the last `::`. So `textstats::analyze` corresponds to `docs/leaves/analyze.md`, `auth::list_providers` to `docs/leaves/list_providers.md`.

## What goes in the leaf file

- A topical H1 (optional). Choose a phrase that names the *task*, not the function. Prefer `# Sizing text before provider calls`, not `# textstats::analyze`.

...
#[test]
fn check_rendered_flags_stale_skills_dir() {
    let tmp = TempDir::new().unwrap();
    write_minimal_worker(tmp.path());
    render_to_disk(tmp.path());
    // A leftover skills/ dir from the pre-inlining layout must be flagged so
    // `--write` removes it.
    ...
    let drift = iii_skill_core::render::check_rendered(tmp.path()).unwrap();
    assert!(
        drift.iter().any(|d| d.contains("skills/") && d.contains("stale")),
        "expected stale skills/ flag, got: {drift:?}"
    );
}
